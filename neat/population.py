...
class Population(object):
    ...
    def __init__(self, checkpoint_file=None):
        ...
        if checkpoint_file:
            ...
            self.__popsize = config.Config.pop_size
    ...
    def __create_population(self):
        ...
        for i in range(self.__popsize):
            ...
            if config.Config.hidden_nodes > 0:
                g.add_hidden_nodes(config.Config.hidden_nodes)
    ...
    def __speciate(self, report):
        ...
        for individual in self:
            ...
            for s in self.__species:
                if individual.distance(s.representant) < config.Config.compatibility_threshold:
                    s.add(individual)
                    found = True
                    break
    ...
    def __set_compatibility_threshold(self):
        ...
        if len(self.__species) > config.Config.species_size:
            config.Config.compatibility_threshold += config.Config.compatibility_change
        elif len(self.__species) < config.Config.species_size:
            if config.Config.compatibility_threshold > config.Config.compatibility_change:
                config.Config.compatibility_threshold -= config.Config.compatibility_change
            else:
    ...
    def __compute_spawn_levels(self):
        ...
        species_stats = []
        for s in self.__species:
            if s.age < config.Config.youth_threshold:
                species_stats.append(s.average_fitness()*config.Config.youth_boost)
            elif s.age > config.Config.old_threshold:
                species_stats.append(s.average_fitness()*config.Config.old_penalty)
            else:
    ...
    def epoch(self, n, report=True, save_best=False, checkpoint_interval = 10,
        ...
        for g in range(n):
            ...
            if best.fitness > config.Config.max_fitness_threshold:
            ...
            for s in self.__species[:]:
                if s.no_improvement_age > config.Config.max_stagnation:
                    if not s.hasBest:
                        if report:
            ...
            for s in self.__species[:]:
                if s.no_improvement_age > 2*config.Config.max_stagnation:
                    if report:
